...
function seed() {
  ...
  sm.forEach(m => {
    const cr = d[m.ci]; const rId = uid(), mId = uid(), at = new Date(Date.now() - m.ago).toISOString();
...
let _authMode = 'login';
function renderAuth() {
  const s = document.getElementById('s-auth'); s.innerHTML = `<div class="auth">
...
async function doAuth() {
  const btn = document.getElementById('abtn'), err = document.getElementById('aerr'); btn.disabled = true; btn.innerHTML = `<span class="spin"></span> ${_authMode === 'signup' ? 'Creating…' : 'Signing in…'}`; err.classList.remove('show');
  try {
    const e = document.getElementById('f-e').value.trim(), p = document.getElementById('f-p').value.trim();
    if (_authMode === 'signup') {
      const u = document.getElementById('f-u').value.trim(); const ue = V.user(u); if (ue) throw new Error(ue); const pe = V.pass(p); if (pe) throw new Error(pe); if (!e || !V.email(e)) throw new Error('Valid email required'); if (S.users.find(x => x.email.toLowerCase() === e.toLowerCase())) throw new Error('Email already exists');
    ...
    await wait(500); go('home')
  } catch (e) { err.textContent = e.message; err.classList.add('show'); btn.disabled = false; btn.textContent = _authMode === 'signup' ? 'Create Account' : 'Login' }
}
...
<div class="ss"><div class="ss-c"><div class="ss-dr"><div class="ss-dot" style="background:#4D7A58"></div><span class="ss-l">Open</span></div><span class="ss-v">${op}</span></div><div class="ss-c"><div class="ss-dr"><div class="ss-dot" style="background:#4D8EFF"></div><span class="ss-l">In Progress</span></div><span class="ss-v">${ip}</span></div><div class="ss-c"><div class="ss-dr"><div class="ss-dot" style="background:#B4FF00"></div><span class="ss-l">Completed</span></div><span class="ss-v">${dn}</span></div></div>
...
function renderMissions() {
  const s = document.getElementById('s-missions'); _allMs = S.missions.map(hydrate);
...
function renderReport() {
  const s = document.getElementById('s-report'); _rImg = null;
...
async function subRpt() {
  const btn = document.getElementById('rbtn'); btn.disabled = true; btn.innerHTML = `<span class="spin"></span> Submitting…`;
  try {
    await wait(1000); const loc = document.getElementById('f-loc').value.trim(), desc = document.getElementById('f-desc')?.value || '';
  ...
  } catch (e) { btn.disabled = false; btn.textContent = 'Submit Report' }
}

/* Detail */
function renderDetail() {
  const s = document.getElementById('s-detail'); const m = S.missions.find(x => x.id === S.detailId); if (!m) { s.innerHTML = '<div class="empty"><div class="empty-t">Not found</div></div>'; s.style.display = 'block'; s.classList.add('active'); return }
...
async function accM(mId) {
  ...
  try {
    await wait(600); const m = S.missions.find(x => x.id === mId); if (!m || m.st !== 'open') throw new Error('Unavailable'); m.st = 'in-progress'; m.ab = S.user.id; save();
    btn.innerHTML = `<span style="color:var(--accent)">${I.ok} Accepted ✓</span>`;
    setTimeout(() => { const area = document.getElementById('parea'); if (area) { area.innerHTML = proofHTML(mId); area.style.opacity = '0'; requestAnimationFrame(() => { area.style.transition = 'opacity .2s'; area.style.opacity = '1' }) } }, 400)
  } catch (e) { btn.disabled = false; btn.innerHTML = I.ok + ' Accept Mission' }
}

async function subPrf(mId) {
  const btn = document.getElementById('pbtn'); if (!_pImg) return; btn.disabled = true; btn.innerHTML = `<span class="spin"></span> Verifying…`;
  ...
  if (ok) {
    m.st = 'approved'; m.ct = now(); S.user.mc = (S.user.mc || 0) + 1; S.user.points = (S.user.points || 0) + 150; S.user.level = Math.floor(S.user.points / 500) + 1; S.log.push({ id: uid(), uid: S.user.id, amt: 150, reason: 'Cleanup approved', at: now() }); save();
...
/* Leaderboard */
function renderLB() {
  const s = document.getElementById('s-leaderboard'); const users = [...S.users].sort((a, b) => b.points - a.points); const t3 = users.slice(0, 3), rest = users.slice(3);
