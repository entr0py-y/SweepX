...
Verdict rules:
- "approved":      cleanup is visible, confidence >= 0.55
...
- "manual_review": very ambiguous, very poor quality, or confidence below 0.40

Important context:
- Before and after photos are taken at DIFFERENT TIMES, so lighting, angle, and minor
  background changes are EXPECTED and should NOT reduce confidence
- Focus on whether garbage/litter is clearly REMOVED, not on exact visual similarity
- A clean ground/area in the after photo IS strong evidence of cleanup
- Give higher confidence when the after image shows a visibly cleaner scene
